...
## Reinforcement loop (v0.4) + human review

**RL-lite (reward from outcomes)**: every submitted task's outcome (accepted/rejected + classified audit reasons) is a learning signal. `extractOutcomes()` reads them from the event log; `updateSkillStats()` adjusts skill utility by real outcomes (cited+accepted ↑, cited+rejected ↓, 14-day half-life decay); `strategyStats()/rankDescriptors()` compute UCB acceptance priors per (phase, descriptor) — the scheduler orders blind-generator seeds by them (exploration bonus prevents starvation). Audit rejection classes (`classifyRejection`: ref-not-found, self-promotion, missing-required-object, weak-summary, bound-mismatch…) flow into every ContextPack as `recentRejections` — workers see past mistakes before repeating them. Result envelopes now carry `skillsUsed` (replay-safe optional field — the citation signal that was silently dropped before).

**Human review**: `research review <campaign>` lists quarantined/promotion-ready discovery candidates + distilled lessons + recent verifications; `research decide <campaign> accept|reject <subjectId>` (or `note <text>`) persists the decision as a `human.intervention` event and flips candidate promotionStatus. The human is the terminal gate of every novelty claim.

## Red-team adversarial verification (v0.6)

When a campaign **completes** (not budget-stops) with verified claims, the supervisor auto-spawns `zz-<problem>-redteam.yaml`: a dedicated adversarial campaign whose SOLE objective is to **destroy** the result. The red-team workers: (1) re-compute independently from scratch (different implementations, no access to original scripts); (2) hunt counterexamples beyond verified bounds; (3) check circular reasoning in verification chains; (4) attack certificates; (5) adversarial perturbation (test n=K+1, K+10, K+100); (6) look for off-by-one/sign/overflow; (7) verify bound-integrity (stated vs actually-verified). If results survive → they earned the **survived-red-team** badge. If destroyed → the precise failure mode is documented for the human.

**Publication pipeline** (next stage after red-team survival): freeze original artifacts → independent re-verification → manuscript skeleton (LaTeX-ready: problem, prior work, theorem, proof/computation, methodology incl. AI-assistance disclosure, reproducibility, references) → private specialist review → arXiv preprint → journal submission. The system NEVER auto-publishes; the human is the terminal gate at every step.
